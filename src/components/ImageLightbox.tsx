...
async function enterMediaFullscreen(element: HTMLElement, item: MediaLightboxItem) {
  ...
  if (!orientation.lock || !item.width || !item.height) return;
  try {
    await orientation.lock(item.width >= item.height ? "landscape" : "portrait");
  } catch {
    // Desktop browsers and iOS may reject orientation locking while still allowing fullscreen.
  }
}
...
export function MediaLightbox({
  ...
  return createPortal(
    <div className="dialog-backdrop message-image-preview-backdrop" onClick={() => {
      ...
      <section aria-modal="true" className="message-image-preview-modal" ref={modalRef} role="dialog">
