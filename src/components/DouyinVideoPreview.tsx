...
export function isDouyinVideoData(value: unknown): value is DouyinVideoDataDTO {
  ...
  try {
    ...
    return (!media || (media.protocol === "https:" && allowedMedia.some((host) => media.hostname === host || media.hostname.endsWith(`.${host}`))))
  ...
  return <>
    ...
    <SideDrawer className="douyin-video-drawer" historyKey={`douyin-video-${video.video_id}`} open={open} onClose={() => setOpen(false)} title={video.title || t("douyin.video")}>
      <div className="douyin-video-viewer">
        {open && video.video_url ? <video title={video.title || t("douyin.video")} src={video.video_url} controls autoPlay playsInline poster={imageUrl} style={{ aspectRatio }} /> : null}
