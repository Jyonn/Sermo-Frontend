...
function writeAccountVault(accounts: StoredPwaAccount[]) {
  ...
  try {
    ...
    // The active scoped session still works if persistent storage is unavailable.
  }
}
...
export function rememberPwaAccountSession(session: AuthSession, slug: string) {
  ...
  const next = { key, slug, session, lastVisitedAt: Date.now() };
  writeAccountVault([next, ...readAccountVault().filter((account) => account.key !== key)]);
}
...
export function listPwaCachedAccounts(): PwaCachedAccount[] {
  ...
  for (let index = 0; index < window.localStorage.length; index += 1) {
    ...
    try {
      ...
      accountMap.set(`${slug}:${session.user.user_id}`, {
        key: `${slug}:${session.user.user_id}`,
        slug,
        session,
        lastVisitedAt: Math.max(recent?.lastVisitedAt || 0, accountMap.get(`${slug}:${session.user.user_id}`)?.lastVisitedAt || 0),
      });
    } catch {
