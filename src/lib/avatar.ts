export const AVATAR_PRESET_TOTAL = 80;
...
const AVATAR_PRESET_BASE_URI = "https://image.6-79.cn/sermo/assets/avatars";
...
export function buildAvatarPresetUri(id: number) {
  return `${AVATAR_PRESET_BASE_URI}/${formatAvatarPresetId(id)}.svg`;
}

export function parseAvatarPresetId(uri?: string | null) {
  if (!uri) return null;
  const match = uri.match(/\/(\d{2})\.svg(?:\?.*)?$/);
