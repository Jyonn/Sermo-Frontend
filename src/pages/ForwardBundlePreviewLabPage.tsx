...
const variants: Array<{ key: Variant; index: string; title: string; note: string }> = [
  { key: "seamless", index: "01", title: "无界嵌入", note: "聊天背景直接接入消息流，最接近普通 Chat Preview" },
  { key: "viewport", index: "02", title: "聊天视窗", note: "只用细边界定义可滚动区域，不形成发送者气泡" },
  { key: "ribbon", index: "03", title: "时间切片", note: "用时间轴标签交代来源，聊天内容保持完整展开" },
  { key: "editorial", index: "04", title: "留白剧场", note: "弱化所有容器感，让内部对话成为视觉主体" },
];
  ...
  return (
    <div className="forward-lab-phone">
      ...
      <main>
        ...
        <div className="forward-lab-inline-context">
...
export default function ForwardBundlePreviewLabPage() {
  ...
  return (
    <main className="forward-lab-page">
      <style>{styles}</style>
      <header className="forward-lab-hero"><span>CHAT PREVIEW / INLINE STUDY</span><h1>不是一张卡片，<br />而是一段聊天。</h1><p>四版均已移除发送者气泡外壳。浅绿色区域就是聊天背景，内部气泡才属于原消息发送者。</p></header>
      ...
      <section className="forward-lab-stage">
        <aside><small>SELECTED DIRECTION</small><strong>{selected.title}</strong><p>{selected.note}</p><ul><li>没有外层发送者气泡</li><li>内部区域直接使用聊天背景</li><li>超过最大高度后区域内滚动</li><li>内部合并记录保持紧凑卡片</li></ul></aside>
...
@media(max-width:760px){.forward-lab-page{padding:22px 12px 40px}.forward-lab-hero{padding:0 8px}.forward-lab-hero h1{font-size:42px}.forward-lab-switcher{grid-template-columns:1fr 1fr}.forward-lab-switcher small{display:none}.forward-lab-stage{display:block;padding:14px;border-radius:28px}.forward-lab-stage aside{padding:12px 8px 22px}.forward-lab-stage aside>strong{font-size:28px}.forward-lab-stage aside ul{display:none}.forward-lab-phone{height:720px;max-height:78dvh;min-height:600px;margin:auto}.forward-lab-inline-context{margin-inline:-12px}}
`;
