...
function PlatformAdminConsole() {
  ...
  const [spaceFeatureBusy, setSpaceFeatureBusy] = useState(false);
  ...
  const toggleQqBindingGrant = async () => {
    if (!selectedSpace || spaceFeatureBusy) return;
    setSpaceFeatureBusy(true);
    try {
      const updated = await api.setPlatformSpaceFeature(selectedSpace.space_id, "qq_identity_binding", !selectedSpace.qq_binding_granted);
      setSelectedSpace(updated);
      setSpaces((items) => items.map((item) => item.space_id === updated.space_id ? updated : item));
      showToast(updated.qq_binding_granted ? "已授予 QQ 绑定能力" : "已撤销 QQ 绑定能力并关闭空间开关");
    } catch (cause) {
      showToast(cause instanceof Error ? cause.message : "功能授权更新失败", "error");
    } finally {
      setSpaceFeatureBusy(false);
    }
  };
  ...
  <SideDrawer historyKey="platform-space" onClose={() => setSelectedSpace(null)} open={Boolean(selectedSpace)} title={selectedSpace?.name || "空间详情"}>{selectedSpace ? <div className="platform-drawer-stack"><section className="platform-space-summary"><UserAvatar className="platform-profile-avatar" name={selectedSpace.name} uri={selectedSpace.official_user?.avatar_uri} /><div><h2>{selectedSpace.name}</h2><p>@{selectedSpace.slug}</p></div><span className={`platform-tier is-${selectedSpace.verification_tier}`}>{selectedSpace.verification_tier}</span></section><section className="platform-data-grid"><span>成员<strong>{selectedSpace.member_count}</strong></span><span>容量<strong>{selectedSpace.member_limit}</strong></span><span>聊天<strong>{selectedSpace.chat_enabled ? "开启" : "关闭"}</strong></span><span>广场<strong>{selectedSpace.square_enabled ? "开启" : "关闭"}</strong></span></section><section className={`platform-feature-grant${selectedSpace.qq_binding_granted ? " is-granted" : ""}`}><span className="material-symbols-outlined">link</span><div><strong>QQ 身份绑定</strong><small>{selectedSpace.qq_binding_granted ? selectedSpace.qq_binding_enabled ? "空间已获授权，管理员已开启" : "空间已获授权，等待管理员开启" : "默认不可用；授予后由空间管理员决定是否开启"}</small></div><button disabled={spaceFeatureBusy} onClick={() => void toggleQqBindingGrant()} type="button">{spaceFeatureBusy ? "处理中" : selectedSpace.qq_binding_granted ? "撤销授权" : "授予空间"}</button></section><section className="platform-panel"><h3>成员</h3>{members.length ? members.map((member) => <button className="platform-member-row" key={member.user_id} onClick={() => setSelectedMember(member)} type="button"><UserAvatar className="platform-member-avatar" name={member.name} uri={member.avatar_uri} /><div><strong>{member.name}</strong><small>LV{member.growth_level ?? 1} · {member.verified ? "已认证" : "未认证"}</small></div><span className="material-symbols-outlined">chevron_right</span></button>) : <div className="platform-inline-loading"><i />正在读取成员</div>}</section></div> : null}</SideDrawer>
